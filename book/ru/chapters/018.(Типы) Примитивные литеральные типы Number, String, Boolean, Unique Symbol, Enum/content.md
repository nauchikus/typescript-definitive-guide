...
Для того чтобы избежать подобный сценарий, _TypeScript_ добавил новый примитивный литеральный тип данных `Unique Symbol`. `Unique Symbol` является подтипом `Symbol` и указывается в аннотации с помощью литерального представления `unique symbol`.
...
if (KEY === OTHER_KEY) {
} // Error, unique symbol не равно unique symbol
...
Тип `unique symbol` предназначен для аннотирования уникальных символьных литералов. С его помощью реализуется задуманное для _JavaScript_ поведение, типизированной среде _TypeScript_.
