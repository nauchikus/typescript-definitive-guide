...
Для примера представьте массив хранящий экземпляры классов `Animal`, `Elephant` и `Lion` последнии два из которых расширяют первый. И кроме того ссылка на данный массив присваивается переменной.
