...
interface GreeterDefaultProps {} // для декларации свойст по умолчанию
export interface GreeterProps extends GreeterDefaultProps {} // для декларации свойст + экспорт интерфейса
...
class ListItem extends Component<ListItemProps, ListItemState> {
render(): ReactElement<ListItemProps> { // уточняем возвращаемы тип
  return <li>{this.props.children}</li>;
}
}
...
function SelectWithData({data, onSelect}:Props):ReactElement<Props>{
  ...
  // внутренний слушатель событий, который отслеживает изменение более низкоуровневого комопнента Select
...
<CustomComponent /> // Error, отсутсвуют обязательные свойства inputProp и outputProp
<CustomComponent inputProp={ '' } /> // Error, отсутсвует обязательное свойство outputProp
...
function withHOC<SharedProps>(WrappedComponent:
  ...
  // тип свойст выходного компонента указан как псевдоним типа
  class WrapperComponent extends Component<InternalInputProps, State, Snapshot> {
      ...
      private inputToOutputProps(inputProps: Readonly<InternalInputProps>, outputProps: OutputProps): SharedProps & OutputProps {
          ...
          let {propInputInHOC, ...sharedProps} = inputProps as any; // фильтруем ствойства
...
<CustomComponent /> // Error, отсутсвуют обязательные свойства sharedProp и outputProps
<CustomComponent sharedProp={ '' } /> // Error, отсутсвует обязательное свойство outputProps
