...
Тип данных `Void` указывается с помощью ключевого слова `void` (не путать с одноимённым оператором из *JavaScript*) и в отличии от таких типов, как `Null` и `Undefined` не имеет никаких значений.
