...
Функциональная примесь — это функция, которая в своем теле ссылается на объявленные в объекте, к которому она “примешивается”, члены. Проблем не возникнет, если подобный механизм реализуется в динамически типизированном языке, каким является *JavaScript*.
