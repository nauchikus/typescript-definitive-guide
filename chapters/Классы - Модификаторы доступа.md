...
В случаях, когда класс состоит только из статических свойств и методов, как, например, класс `Math`, его конструктор можно пометить, как приватный, тем самым запретить создавать его экземпляры.
