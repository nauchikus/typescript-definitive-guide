...
На последок стоит упомянуть об одном не очевидном моменте. Оператор `keyof` можно совмещать с оператором `typeof` (Type Queries).
...
type T0<T> = { -readonly [P in keyof T]-?: T[P] }; // удалит указынные модификаторы
